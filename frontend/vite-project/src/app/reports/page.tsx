...
import { FileText, Download, Eye, RefreshCw, FileQuestion } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not fetch reports from server.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl w-full mx-auto flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between border-b border-border pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-foreground">Executive Security Reports</h1>
              <p className="text-xs text-muted-foreground">View and download generated markdown executive security assessments stored in MongoDB Atlas.</p>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-border bg-card text-xs font-medium text-foreground hover:bg-muted"
          >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 rounded-2xl border border-border bg-card text-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 max-w-sm">
                No security reports yet. Run a File Analysis or Log Analysis to generate your first report.
              </p>
            </div>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Created: {new Date(rep.created_at).toLocaleString()} • Format: {rep.format.toUpperCase()}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-border bg-muted/50 text-xs font-medium text-foreground hover:bg-muted"
                  >
                    <Eye size={14} />
                    <span>View</span>
                  </button>

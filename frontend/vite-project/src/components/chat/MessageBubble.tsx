...
import { Copy, RefreshCw, User, Check, Sparkles, ArrowUpRight } from "lucide-react";
...
const MessageBubble: React.FC<Props> = ({
  ...
  return (
    <div
      className={cn(
        "group flex w-full flex-col gap-1.5 py-2 text-xs sm:text-sm animate-message-enter motion-reduce:animate-none",
    ...
    >
      ...
      <div className="flex items-center gap-1.5 px-1 text-[11px] text-muted-foreground">
        {isUser ? (
          ...
          <img
            ...
            alt="SentinelForge AI"
            className="h-3.5 w-3.5 object-contain shrink-0 rounded-xs"
          />
        )}
      ...
      <div
        className={cn(
          "relative max-w-[90%] sm:max-w-[85%] rounded-md px-4 py-3 shadow-xs break-words overflow-hidden transition-all duration-normal",
          isUser
      ...
      {/* Footer / Actions */}
      <div className="flex items-center gap-2 px-1 text-[11px] text-muted-foreground min-h-[22px]">
      ...
      {/* Contextual Explore Next Compact Rows */}
      {isAssistant && message.status === "complete" && isLatestAssistantMessage && onSelectPrompt && followUpQuestions.length > 0 && (
        <div className="w-full max-w-[90%] sm:max-w-[85%] mt-2 pt-3 border-t border-border/50 flex flex-col gap-2 animate-message-enter">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full">
            {followUpQuestions.map((q, idx) => (
              <button
                ...
                className="flex items-center justify-between gap-2 p-2.5 rounded-md border border-border bg-card hover:bg-card-surface hover:border-primary/40 hover:-translate-y-[1px] transition-all duration-normal text-left text-xs font-medium text-foreground cursor-pointer group shadow-2xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
              >
